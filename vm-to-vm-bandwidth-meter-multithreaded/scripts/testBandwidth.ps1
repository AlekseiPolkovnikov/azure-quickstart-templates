...
if (!(Test-Path $AppPath)){
    ...
    New-NetFirewallRule -DisplayName "Allow NTttcp In" -Direction Inbound -Program "$NTttcpPath\NTttcp.exe" -RemoteAddress LocalSubnet -Action Allow
    New-NetFirewallRule -DisplayName "Allow NTttcp Out" -Direction Outbound -Program "$NTttcpPath\NTttcp.exe" -RemoteAddress LocalSubnet -Action Allow

}

if ($DataTransferMode = "Async") {$dtmode = "-a"}

if ($Mode -eq "Sender"){$srmode = "-s"}
else {$srmode = "-r"}


& "$NTttcpPath\NTttcp.exe" $srmode $dtmode -l $BufferSize -m "$ThreadNumber,*,$ReceiverIP" -a $OverlappedBuffers -t $Duration
